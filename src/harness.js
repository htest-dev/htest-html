...
function runSelected (names) {
	...
	function updateTotals () {
		...
		let totalsEl =
			document.querySelector("h1 + .totals") ||
			create({ className: "totals", after: document.querySelector("h1") });
...
for (let a of tests.querySelectorAll("li > a")) {
	...
	a.insertAdjacentHTML(
		"afterend",
		`<a href="${a.getAttribute("href")}" class="new-tab" title="Open in new Tab" target="_blank">↗️</a>`,
	);
	...
	li.addEventListener("click", evt => {
		if (evt.target.matches("a")) {
			...
			let checked = $$("input[type=checkbox]:not(#select_all)", tests).map(
